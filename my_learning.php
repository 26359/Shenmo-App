...
$my_courses = $conn->query("
    SELECT e.*, c.course_name, c.description, c.level_number, c.duration_weeks
    FROM enrollments e
...
if ($my_courses && $my_courses->num_rows > 0) {
    while ($course = $my_courses->fetch_assoc()) {
        $progress_result = $conn->query("
            ...
            WHERE sp.student_id = '$student_id' AND sp.course_id = " . $course['course_id'] . "
        ");
        $course_progress[$course['course_id']] = $progress_result ? $progress_result->fetch_assoc() : null;
    }
...
<head>
    <meta charset="UTF-8">
    <title>My Learning - Abacus Academy</title>
    <style>
    ...
    <div class="dashboard">
        <aside class="sidebar">
            ...
            <ul class="nav-menu">
                ...
                <li class="nav-item">
                    <a href="my_learning.php" class="nav-link active">
                        <span class="icon">📚</span>
            ...
            <div class="courses-list">
                <?php if ($my_courses && $my_courses->num_rows > 0): ?>
                    <?php while($course = $my_courses->fetch_assoc()): 
                        $progress_data = $course_progress[$course['course_id']] ?? null;
                        $progress_percentage = $progress_data ? round($progress_data['progress_percentage']) : 0;
                    ?>
                        <div class="course-card">
                            <div class="course-header">
                                <div class="course-info">
                                    <h3>Level <?php echo $course['level_number']; ?> - <?php echo htmlspecialchars($course['course_name']); ?></h3>
                            ...
                            <div class="course-progress">
                                ...
                                <div style="color: #64748b; font-size: 0.85rem; margin-top: 8px;">
                                    <?php echo $progress_data ? $progress_data['completed_lessons'] : 0; ?> of <?php echo $progress_data ? $progress_data['total_lessons'] : 0; ?> lessons completed
                                </div>
                            </div>
                            
                            <div class="course-actions">
                                <a href="course_viewer.php?course_id=<?php echo $course['course_id']; ?>" class="btn btn-primary">
                                    📖 Continue Learning
                                </a>
                            </div>
                        </div>
                ...
                <?php else: ?>
                    <div class="no-data">
                        ...
                        <p>Browse the course catalog to start your learning journey!</p>
                        <a href="course_catalog.php" class="btn btn-primary" style="margin-top: 20px;">Browse Courses</a>
                    </div>
                <?php endif; ?>
            </div>
        </main>
